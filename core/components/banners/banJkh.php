...
add_action( 'init', 'true_register_post_type_init_banjkh' ); // Использовать функцию только внутри хука init

function true_register_post_type_init_banjkh() {
    $labels = array(
        'name' => 'ЖКХ - элементы баннера',
        ...
        'menu_name' => 'ЖКХ баннер' // ссылка в меню в админке
    );
    $args = array(
        'labels' => $labels,
        'public' => true,
    ...
    register_post_type('banjkh', $args);
}
